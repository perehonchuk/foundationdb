...
struct AddingShard : NonCopyable {
	...
	enum Phase {
		...
		FetchingCF,
		// During Validating phase, it verifies data integrity and prepares the transferred version.
		Validating,
...
ACTOR Future<Void> fetchKeys(StorageServer* data, AddingShard* shard) {
	...
	try {
		...
		// Enter Validating phase before choosing transferredVersion
		shard->phase = AddingShard::Validating;

		TraceEvent(SevDebug, "FetchKeysValidating", data->thisServerID)
		    .detail("FKID", interval.pairID)
		    .detail("UpdatesSize", shard->updates.size())
		    .detail("BatchChangesSize", batch->changes.size());

		// Choose a transferredVersion using the first update's version if available,
		// otherwise fall back to data->version.get() + 1. This ensures better alignment
		// with actual mutation timing.
		if (!shard->updates.empty() && batch->changes.size() > 0) {
			shard->transferredVersion = std::min(batch->changes[0].version, shard->updates[0].version);
		} else if (!shard->updates.empty()) {
			shard->transferredVersion = shard->updates[0].version;
		} else if (batch->changes.size() > 0) {
