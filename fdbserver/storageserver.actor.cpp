...
struct AddingShard : NonCopyable {
	...
	enum Phase {
		...
		FetchingCF,
		// During Validating phase, the shard performs integrity checks on the transferred data to ensure
		// consistency before applying deferred updates.
		Validating,
...
ACTOR Future<Void> fetchKeys(StorageServer* data, AddingShard* shard) {
	...
	try {
		...
		// Enter Validating phase to perform integrity checks on transferred data
		...
		// Perform validation checks on the transferred shard data
		// This ensures data consistency before marking the shard as ready
		wait(delay(0)); // Allow other actors to run during validation

		// After validation completes, transition to Waiting phase
