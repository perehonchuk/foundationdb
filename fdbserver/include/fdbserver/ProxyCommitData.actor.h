...
struct ProxyStats {
	...
	Reference<Histogram> preCommitValidationDist;
	...
	explicit ProxyStats(UID id,
	  ...
	  : cc("ProxyStats", id.toString()), txnCommitIn("TxnCommitIn", cc),
	    ...
	    preCommitValidationDist(
	        Histogram::getHistogram("CommitProxy"_sr, "PreCommitValidation"_sr, Histogram::Unit::milliseconds)),
