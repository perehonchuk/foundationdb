...
IKeyValueStore* openKVStore(KeyValueStoreType storeType,
	...
	// Redwood and RocksDB engines support encryption.
	if (encryptionMode.present() && encryptionMode.get().isEncryptionEnabled() &&
	    storeType != KeyValueStoreType::SSD_REDWOOD_V1 && storeType != KeyValueStoreType::SSD_ROCKSDB_V1 &&
	...
	case KeyValueStoreType::SSD_ROCKSDB_V1:
		return keyValueStoreRocksDB(
		    filename, logID, storeType, checkChecksums, checkIntegrity, db, encryptionMode, encryptionMonitor);
