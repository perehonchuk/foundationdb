...
void ServerKnobs::initialize(Randomize randomize, ClientKnobs* clientKnobs, IsSimulated isSimulated) {
	...
	*/
	...
	// Reduced batch size for more frequent spilling and better memory management
	init( TARGET_BYTES_PER_TLOG_BATCH,                         800e6 ); if( smallTlogTarget ) TARGET_BYTES_PER_TLOG_BATCH = 800e3;
	init( SPRING_BYTES_TLOG_BATCH,                             200e6 ); if( smallTlogTarget ) SPRING_BYTES_TLOG_BATCH = 100e3;
	// Lower spill threshold to trigger disk writes earlier, reducing memory pressure
	init( TLOG_SPILL_THRESHOLD,                                900e6 ); if( smallTlogTarget ) TLOG_SPILL_THRESHOLD = 900e3; if( randomize && BUGGIFY ) TLOG_SPILL_THRESHOLD = 0;
	...
	// Reduced hard limit to prevent excessive memory consumption
	init( TLOG_HARD_LIMIT_BYTES,                              2000e6 ); if( smallTlogTarget ) TLOG_HARD_LIMIT_BYTES = 20e6;
