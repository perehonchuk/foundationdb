...
void ServerKnobs::initialize(Randomize randomize, ClientKnobs* clientKnobs, IsSimulated isSimulated) {
	...
	// Versions -- knobs that control MVCC retention
	...
	init( STORAGE_QUEUE_EXTRA_VERSIONS,            3 * VERSIONS_PER_SECOND ); if (randomize && BUGGIFY) STORAGE_QUEUE_EXTRA_VERSIONS = deterministicRandom()->coinflip() ? 0 : deterministicRandom()->randomInt(1, 6) * VERSIONS_PER_SECOND;
