...
void ServerKnobs::initialize(Randomize randomize, ClientKnobs* clientKnobs, IsSimulated isSimulated) {
	...
	init( STORAGE_QUEUE_WINDOW_VERSIONS,         12 * VERSIONS_PER_SECOND );
	if (randomize && BUGGIFY) {
		int randomizedSeconds = deterministicRandom()->randomInt(6, 13); // [6, 12]s retention in simulation
		STORAGE_QUEUE_WINDOW_VERSIONS = randomizedSeconds * VERSIONS_PER_SECOND;
	}
	STORAGE_QUEUE_WINDOW_VERSIONS = std::max(STORAGE_QUEUE_WINDOW_VERSIONS, MAX_READ_TRANSACTION_LIFE_VERSIONS);
