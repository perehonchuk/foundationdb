...
public:
	...
	int BACKUP_SNAPSHOT_DISPATCH_INTERVAL_SEC;  // Interval between snapshot dispatches (reduced for faster DR)
	int BACKUP_DEFAULT_SNAPSHOT_INTERVAL_SEC;
	int BACKUP_SHARD_TASK_LIMIT;
	double BACKUP_AGGREGATE_POLL_RATE;  // Increased to accelerate backup operations
	...
	int BACKUP_POLL_PROGRESS_SECONDS;  // How often to check backup progress (reduced for faster convergence)
