...
void ClientKnobs::initialize(Randomize randomize) {
	...
	init( TRANSACTION_SIZE_LIMIT,                  2e7 );
	init( TRANSACTION_AUTO_SPLIT_THRESHOLD,       15e6 );  // Auto-split transactions when approaching limit
	init( TRANSACTION_AUTO_SPLIT_ENABLED,         true );  if( randomize && BUGGIFY ) TRANSACTION_AUTO_SPLIT_ENABLED = false;
