...
void ClientKnobs::initialize(Randomize randomize) {
	...
	// Versions -- knobs that control transaction lifetime limits
	init( VERSIONS_PER_SECOND,                     1e6 ); // Must be the same as SERVER_KNOBS->VERSIONS_PER_SECOND
	init( MAX_WRITE_TRANSACTION_LIFE_VERSIONS,     8 * VERSIONS_PER_SECOND);  // Must be the same as SERVER_KNOBS->MAX_WRITE_TRANSACTION_LIFE_VERSIONS
